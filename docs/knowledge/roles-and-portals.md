...
| `brand` (Angelica) | Brand and communication | `G-xx` | `/brand` | G-01..G-07 (built) | competitions calendar (by submission date, materials per entry), sales presentations, brand identity rules, client image sets, graphic revisions queue, asset library | `projects.read`, `tasks.own.write`, `brand.manage`, `competitions.manage`, `presentations.write`, `images.write`, `revisions.manage`, `assets.manage`, `leads.read`, `engagements.read`, `manual.read`, `docs.read` |
