...
`docs/screenshots/HUB-01/` — `en-390.jpg`, `en-1280.jpg`, `es-1280.jpg`, `en-1920.jpg` captured live against `/`; `en-3840.jpg` re-captured in changelog 0021 (ar-18, local preview of 0.13.0) with the icon system: four portal cards per row, every "No preview yet" tile drawing its surface icon, read back at full size in `docs/qa/0004-archive-4k-dark.md`; `en-1280-icons.jpg` and `en-1280-icons-dark.jpg` captured against a local `npm run preview` after the icon system landed (every card now carries the icon of the surface it opens, and the "No preview yet" tile draws that icon instead of the monogram); `en-1280.jpg` re-captured in changelog 0019 (version 0.11.0, local preview) after the S-12 Project archive card joined the Product surfaces grid (the card sits below the first viewport; the smoke in `docs/qa/0003-archive-smoke.md` confirms `[data-surface="archive"]` renders as Live). Captured against a local production build (`npm run preview`, same commit as the deploy, version 0.10.0) rather than the live `https://imagine-os.github.io/aluzina/` URL directly: the sandbox's TLS-weakening guard refused the Chromium launch flag needed to make headless Chromium trust the outbound proxy's certificate for that external host. Content is identical (GitHub Pages serves this same build); only the deploy-time hub-card thumbnails (`npm run thumbs`, a separate CI step not run here) are missing locally, so HUB-01 and D-07 show the "No preview yet" placeholder tile instead of real thumbnails in these captures. No overflow or blank-render defects found at the widths captured.
...
3b. Every card in every grid (portals, product surfaces, dev tools, prototype pages) carries the icon of the surface it opens, resolved from its page code (`resolveIcon`, `docs/design/icons.md`): small next to the code in the header line, and large inside the "No preview yet" tile in place of the brand monogram when no thumbnail has been generated. The four portal homes are deliberately distinct (A-01 dashboard, O-01 execution, S-01 design, G-01 brand, C-01 home) so the portal row never reads as four copies of one shape. Icons are decorative (`aria-hidden`): code, title, description and status still carry the meaning.

4. **Product surfaces** grid (`auto-fill, minmax(18rem, 1fr)`; status derived from the route manifest by code: built -> Live, stub -> Stub, no route -> Planned): P-00 Public website (external, aluzinaa.com), P-01 Services and intake (`#/services`, 0013), G-08 Portfolio & brochure (`#/brand/documents`; switches to the brand demo user when the current role lacks `brand.manage`, `enterUnless`, so the card never lands on a permission wall), C-01 Client app (`#/client`), M-01 Ops manual (`#/manual`), D-06 Docs (`#/docs`, the in-app viewer since 0013; GitHub link while no route), S-12 Project archive (`#/studio/archive`; switches to the studio demo user when the current role lacks `projects.read`, `enterAs: 'studio'` / `enterUnless`, changelog 0019), K-01 Spaces (opens on the current role's surface; founder when the role has none), BOS-01 Business OS prototype. Each card starts with a 16 / 10 thumbnail (`./thumbs/<code>.jpg`, 640 x 400) or the bilingual "No preview yet" tile.
