...
files: docs/prompts/0008-social-scraping-instagram-intake.md, docs/changelog/0012-instagram-intake-attempt.md, docs/decisions.md (D-032), docs/knowledge/social-channels.md (new), docs/knowledge/README.md, docs/source/social/instagram/2026-09-21-aluzina.espacios/{README.md,profile.json,bio_link_site.json} (new; README.md and profile.json corrected 04:40 UTC), docs/source/social/instagram/2026-09-21-aluzinaa/{README.md,profile.json,posts.json,raw/probe_log.txt} (new, correction), docs/README.md, docs/kanban.md, docs/reference/surfaces.md
...
## Correction (2026-09-21 04:40 UTC)

- **Withdrawn inference.** The earlier body and D-032's "Why" read Instagram's `httpErrorPage` / `PolarisErrorRoot` shell for `/aluzina.espacios/` as its 404 page and therefore as evidence that the handle does not exist. That is wrong: the capture worker's control fetch of the certainly-live profile `/aluzinaa/` at 04:36:09Z (HTTP 200, 626 KB, `2026-09-21-aluzinaa/raw/control_aluzinaa_shell.html` in the session scratch; the file is described in `2026-09-21-aluzinaa/README.md`) carries the identical markers (`polarisRouteConfig.pageID = "httpErrorPage"`, root `PolarisErrorRoot`, no og: tags, counts or post edges). Instagram serves that shell to every unauthenticated datacenter client from this egress, so it says nothing about whether a username exists. The earlier body above is left as written; this section supersedes its "Handle evidence" first bullet, and D-038 records the correction without editing D-032.
- **What remains** is circumstantial only: https://aluzinaa.com/ links "Síguenos" / "Ver Instagram" to `instagram.com/aluzinaa`, and no search engine indexes `instagram.com/aluzina.espacios`. The mirror's "does not exist" string is generic and was already marked weak. `aluzina.espacios` may still be a renamed, misspelled or private account.
- **@aluzinaa attempt** (`docs/source/social/instagram/2026-09-21-aluzinaa/`, window 04:15:45Z to 04:37:15Z): three curl probes ten minutes apart (`raw/probe_log.txt`): 04:15:45Z 302 -> `/accounts/login/?next=%2Faluzinaa%2F&is_from_rle`, 04:26:04Z 302 -> same, 04:36:09Z **200** (the shell above). The one permitted headless-browser attempt at 04:36:32Z was answered **HTTP 429** with the same login redirect; the script stopped at once, no retry, no login. **0 posts, 0 media, no profile fields for either handle.** The curl 200 was not a usable signal that a browser session would be admitted (different rate-limit buckets).
- **Source folder added:** `2026-09-21-aluzinaa/{README.md,profile.json,posts.json,raw/probe_log.txt}` copied verbatim from the session scratch; its `../README.md` / `../profile.json` references mean the sibling folder `2026-09-21-aluzina.espacios/` here. The 626 KB shell HTML, screenshot and scripts stay in the scratch. Corrected `README.md` and `profile.json` re-copied into `2026-09-21-aluzina.espacios/` (explicit "Correction (04:40Z)" paragraph, struck-through point 1).
- Docs updated in this correction: `knowledge/social-channels.md` (evidence marked withdrawn, attempt added, change-log line), `decisions.md` (D-038), `kanban.md` (Doing card). `npm run build` green before push. Model: Fable 5.1.
