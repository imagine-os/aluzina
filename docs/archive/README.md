...
- `index.json` — the **inventory**: one entry per project folder of Aluzina's Dropbox (187 folders across 8 year folders: 2026 (18), 2025 (37), 2024 (43), 2022 (25), 2021 (23), 2020 (25), 2019-2023 (14), root (2)), with its direct children (1225 files, 243 subfolders, 18.8 GB of listed sizes), inferred `kind` (184 project, 2 quotation-only = prospect, 1 admin), `year`, notes on duplicates / empty / unlisted folders (0 unlisted, 15 empty). Read by `apps/hub/src/data/seed/archive.ts` through `@docs`.
- `projects/<slug>/index.json` — the **deep index** of a featured project: every file with its folder path, size, date, share link, and for design deliverables the served thumbnail and page renders (`apps/hub/public/archive/<slug>/{thumbs,pages}/`, <= 640 px / <= 1200 px, <= 8 pages). `joe-gallina-interior`: 179 files, 25 with a thumbnail, 86 page renders, 147 redacted.
...
Counts this run: 200 of 1225 inventory files redacted; 142 of 179 deep-index files redacted by name / folder and 5 by content; 12 folder segments rewritten; 347 share links replaced by the link of a safe ancestor folder (R6: Dropbox links embed the file path, so a redacted file links to its folder, never to itself). Self-check R7 (names, paths, links, URL-encoded too): passed over 1468 inventory entries and 179 deep-index files.
