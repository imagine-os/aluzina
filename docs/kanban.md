...
- Silver switch: confirm the Pantone / hex with the founder (candidate 877 C), then `metalDefault: 'silver'` + `npm run tokens` and re-derive `--color-accent*`, `--color-hairline`, `--color-metal-text` per theme when it ships (D-039) (Fable 5.1)
