...
export const homeSpec = defineSpec({
  ...
  layout: [
    ...
    'Card: service delivery — two StatTiles linking to S-10 (open engagements) and S-11 (open revisions)',
  ],
  dataTables: ['projects', 'consistencyChecks', 'tasks', 'renderPacks', 'engagements', 'revisionItems'],
  ...
  logic: [
    ...
    'The service delivery row counts open engagements (started / in-progress) and open revision items (status revision) and opens S-10 / S-11.',
  ],
  ...
  actions: [
    ...
    { id: 'studio.openChecklist', label: 'Open service checklist', intent: 'open the service checklist', permission: 'engagements.read' },
    { id: 'studio.openRevisions', label: 'Open revision matrix', intent: 'open the revision matrix', permission: 'revisionMatrix.write' },
  ],
  checkedAt: WIDTHS,
});
...
export const checklistSpec = defineSpec({
  code: 'S-10',
  name: 'Service checklist',
  purpose:
    'The per-project tracker over the playbook service being delivered (D-033): every phase of the contracted service with its real checklist items, the phase the engagement is on, the approval gate that guards construction (G-06 / G-12) and the strategic brief the work was built from.',
  surface: 'studio',
  navGroup: 'design',
  layout: [
    'PageHeader with the Brief button (opens the strategic brief Drawer)',
    'Project Select over the projects that have an engagement (deep-linked as /studio/checklist/:projectId)',
    'Card: service name, outcome, ladder word, current phase, pipeline status, start / delivery dates, approval',
    'Three StatTiles: current phase progress, overall progress, position in the service',
    'One Card per phase (accordion, the current phase open): Checkbox per playbook item, phase notes, "Open in …" link where the stage has a page, "Mark phase complete" and "Advance to next phase"',
    'Approval gate Card before stage 03-19 of service 03: the rule, the project approval and "Send to procurement"',
    'Drawer: the strategic brief answers, read-only',
  ],
  ...
  logic: [
    ...
    'Approval gate (G-06 / G-12): construction may begin only after the client approves the final design. Send to procurement writes projects.pipelineStatus = procurement and is enabled only while approval is client-approved and the project has not passed procurement yet.',
    'Stages that already have a page in the OS link to it instead of duplicating it: 10 → S-11, 12 / 13 → S-04, 15 → S-06, 16 → S-07, 17 → S-05.',
    'Every write carries basedOn (the row updated_at the person was looking at) so a concurrent edit raises the D-024 conflict notice.',
  ],
  components: ['PageHeader', 'Select', 'Card', 'KeyValue', 'StatTile', 'Checkbox', 'Badge', 'StatusPill', 'Button', 'Drawer', 'EmptyState'],
  actions: [
    { id: 'studio.selectChecklistProject', label: 'Choose project', intent: 'show the service checklist of {project}', permission: 'engagements.read', params: { project: 'id' } },
    { id: 'studio.togglePhase', label: 'Expand phase', intent: 'expand or collapse the phase {phase}', permission: 'engagements.read', params: { phase: 'string' } },
    { id: 'studio.tickItem', label: 'Tick checklist item', intent: 'tick item {item} of phase {phase}', permission: 'engagements.write', params: { phase: 'string', item: 'number', ok: 'boolean' } },
    { id: 'studio.completePhase', label: 'Mark phase complete', intent: 'mark the phase {phase} complete', permission: 'engagements.write', params: { phase: 'string' } },
    { id: 'studio.advancePhase', label: 'Advance to next phase', intent: 'advance this engagement to the next phase', permission: 'engagements.write' },
    { id: 'studio.openBrief', label: 'Brief', intent: 'show the strategic brief of this engagement', permission: 'engagements.read' },
    { id: 'studio.openStagePage', label: 'Open in …', intent: 'open the page that owns the stage {stage}', permission: 'engagements.read', params: { stage: 'string' } },
    { id: 'studio.sendToProcurement', label: 'Send to procurement', intent: 'send the approved design of this project to procurement', permission: 'engagements.write', params: { project: 'id' } },
  ],
  checkedAt: WIDTHS,
});
...
export const revisionsSpec = defineSpec({
  code: 'S-11',
  name: 'Revision matrix',
  purpose:
    'The single revision matrix of stage 10 of service 03 (G-05): every comment from the client, the studio and the founder in one table per project with its validation status, so design changes are never scattered through WhatsApp.',
  surface: 'studio',
  navGroup: 'design',
  layout: [
    'PageHeader with the Placeholder "Send to client" and "Export matrix"',
    'FilterBar: project, stage, status, source',
    'Three StatTiles: open revisions, approved, approved with adjustments',
    'Card (when the matrix is decided): the client can approve for execution in the client app',
    'Card: DataTable of the matrix (stage, item, comment, source, author, status, decided at)',
    'Card: add item form (stage Select from the service phases, item, comment)',
    'Drawer: edit the comment, set the validation status, resolve as adjustment',
  ],
  dataTables: ['revisionItems', 'projects', 'engagements'],
  roles: ['studio', 'founder', 'ops'],
  logic: [
    'G-05: one matrix per project. Rows are revisionItems; nothing is stored as free text on the project.',
    'The stage Select offers the phases of the project’s service (from the playbook) plus every label already used on the matrix, so the matrix stays comparable across projects without a new entity.',
    'A new row is signed with the writer’s role: founder → founder, client → client, everyone else → studio; it starts as revision with no decidedAt.',
    'Setting a status writes decidedAt = today, except revision, which clears it; "Resolve as adjustment" is the approved-with-adjustments shortcut.',
    'Export matrix builds a real CSV (BOM + CRLF, RFC 4180 quoting) of the filtered rows and downloads it through a Blob URL.',
    'When no row is left in revision and the project approval is awaiting-founder or approved, the page says the client can approve for execution in the client app (C-03).',
  ],
  components: ['PageHeader', 'FilterBar', 'Select', 'Input', 'Textarea', 'StatTile', 'Card', 'DataTable', 'StatusPill', 'Badge', 'Drawer', 'KeyValue', 'Button', 'Placeholder', 'EmptyState'],
  actions: [
    { id: 'studio.selectRevisionProject', label: 'Choose project', intent: 'show the revision matrix of {project}', permission: 'revisionMatrix.write', params: { project: 'id' } },
    { id: 'studio.filterRevisions', label: 'Filter matrix', intent: 'filter the matrix by stage {stage}, status {status} and source {source}', permission: 'revisionMatrix.write', params: { stage: 'string', status: 'enum:approved|approved-with-adjustments|revision', source: 'enum:client|studio|founder' } },
    { id: 'studio.addRevisionItem', label: 'Add item', intent: 'add the comment {comment} about {item} at stage {stage} to the matrix', permission: 'revisionMatrix.write', params: { stage: 'string', item: 'string', comment: 'string' } },
    { id: 'studio.editRevisionItem', label: 'Save comment', intent: 'change the comment of the matrix item {item}', permission: 'revisionMatrix.write', params: { item: 'id', comment: 'string' } },
    { id: 'studio.setRevisionStatus', label: 'Set status', intent: 'set the matrix item {item} to {status}', permission: 'revisionMatrix.write', params: { item: 'id', status: 'enum:approved|approved-with-adjustments|revision' } },
    { id: 'studio.exportRevisionMatrix', label: 'Export matrix', intent: 'export the revision matrix as a CSV file', permission: 'revisionMatrix.write' },
    { id: 'studio.sendMatrixToClient', label: 'Send to client', intent: 'send the revision matrix to the client', permission: 'revisionMatrix.write', params: { project: 'id' } },
  ],
  checkedAt: WIDTHS,
});
