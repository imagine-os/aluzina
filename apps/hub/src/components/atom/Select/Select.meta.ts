...
export default defineMeta({
  ...
  props: { label: 'string', options: '{ value, label, disabled? }[]', placeholder: 'string? – empty first option', hint: 'string?', error: 'string?', hideLabel: 'boolean?', '...rest': 'native select attributes (value, onChange; required renders the * marker like Input)' },
