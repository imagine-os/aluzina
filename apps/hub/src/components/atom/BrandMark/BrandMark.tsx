...
export interface BrandMarkProps {
  ...
  glyph?: GlyphName;
  /** Paint. Glyphs are outlines: the finish colours their stroke. Default `metal`. */
...
const DESCRIPTOR_TEXT = ['UNIVERSO', 'DE DISEÑO'];

/** Gradient defs painted with the CSS vars, so they follow theme and the gold / silver switch. */
function Defs({ id, horizontal }: { id: string; horizontal: boolean }) {
  return (
    <defs>
      <linearGradient id={`${id}-metal`} x1="0" y1="0" x2="1" y2="1">
        <stop offset="0" style={{ stopColor: 'var(--metal-base)' }} />
        <stop offset="0.22" style={{ stopColor: 'var(--metal-highlight)' }} />
        <stop offset="0.48" style={{ stopColor: 'var(--metal-base)' }} />
        <stop offset="0.72" style={{ stopColor: 'var(--metal-highlight)' }} />
        <stop offset="1" style={{ stopColor: 'var(--metal-base)' }} />
      </linearGradient>
...
export function BrandMark({ kind, glyph, finish = 'metal', tone, size = 'md', label, className }: BrandMarkProps) {
  const id = useId().replace(/:/g, '');
  const a11y = label ? { role: 'img' as const, 'aria-label': label } : { 'aria-hidden': true as const };
  const classes = cx('brand-mark', `brand-mark--${kind}`, `brand-mark--${size}`, `brand-mark--${finish}`, className);
    ...
    return (
      <span className={classes} {...a11y}>
        <svg className="brand-mark__svg" viewBox={slash.viewBox} focusable="false">
          <path d={slash.paths[0]} className="brand-mark__stroke" style={{ stroke: 'var(--color-metal-text)' }} />
        </svg>
        <span className="brand-mark__descriptor" aria-hidden="true">
          {DESCRIPTOR_TEXT.map((line) => (
  ...
  if (kind === 'monogram') {
    const stemFill = tone ? `var(--brand-${tone})` : paint(id, finish);
    const outline = finish === 'outline';
    return (
  ...
  if (kind === 'glyph') {
    ...
    return (
      <svg className={cx(classes, 'brand-mark__svg')} viewBox={shape.viewBox} focusable="false" {...a11y}>
        <Defs id={id} horizontal={false} />
        {shape.paths.map((d, i) => (
          <path key={i} d={d} className="brand-mark__stroke" style={{ stroke: paint(id, finish === 'outline' ? 'flat' : finish) }} />
        ))}
      </svg>
    );
  }
  ...
  return (
    <svg className={cx(classes, 'brand-mark__svg')} viewBox={wordmark.viewBox} focusable="false" {...a11y}>
      ...
      <path d={wordmark.paths[0]} className={outline ? 'brand-mark__stroke' : undefined} fill={outline ? 'none' : paint(id, finish)} />
    </svg>
  );
}
